...
| -------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------- |
| `POST /auth/register`            | [02-register.md](./02-register.md) (STEP 5.1–5.6)        | 201, user tạo + email gửi                                      | 400, 409                                 |
...
- **TokenService trong transaction của `AuthService.register()`** — `TokenService` (01-setup.md STEP 7) tự inject `PrismaService` riêng, không tham gia chung transaction với `AuthService.register()` được — 02-register.md STEP 5.5 dùng 1 helper viết tay trùng lặp code nhỏ để giải quyết tạm. Cải tiến tương lai: refactor `TokenService` nhận `tx` qua tham số thay vì tự inject `this.prisma`. **Không bắt buộc cho MVP.**
