...
| **`grill-with-docs` → `to-spec`** *(bước kế tiếp, không phải skill riêng)* | | |
...
*(3 skill trên nằm ngoài 17+8 = 25 skill "engineering + productivity" nêu ở trên, tổng cộng repo cài **28 skill dùng được**; 6 skill còn lại trong `skills/in-progress/` không được khai trong `plugin.json` nên chưa cài.)*
