...
- Swagger: `@ApiTags()` đặt tên đầy đủ ý nghĩa (không viết tắt trần như `'auth'`), khai `type:` cho response để OpenAPI đúng contract. **Không sửa Postman thủ công** — Swagger (`/api-json`) là nguồn chuẩn, sync bằng `npm run postman:sync`.
