...
async verifyEmail(dto: VerifyEmailDto): Promise<{ message: string }> {
  ...
  if (this.tokenService.hashRawToken(dto.code) !== record.tokenHash) {
    ...
    // trong thời gian hiệu lực. Dùng updateMany với điều kiện
    // `attempts: { lt: MAX_VERIFY_ATTEMPTS }` thay vì update thường: nếu
    // nhiều request đoán sai đến gần như đồng thời lúc attempts đang ở
    // ngưỡng gần giới hạn, mỗi request đọc `record.attempts` ở trên đều có
    // thể còn dưới ngưỡng, dẫn tới tất cả cùng tăng và `attempts` vượt quá
    // MAX_VERIFY_ATTEMPTS. Đặt điều kiện ngay trong WHERE để DB tự đảm bảo
    // atomic, không dựa vào giá trị `record.attempts` đã đọc trước đó.
...
| `hashRawToken(dto.code) !== record.tokenHash` → `updateMany({ where: { id, attempts: { lt: MAX_VERIFY_ATTEMPTS } }, ... })` | Code sai → tăng attempts, 404      | Đếm cả lần đoán sai để giới hạn có tác dụng thực tế. Điều kiện `attempts: { lt: MAX_VERIFY_ATTEMPTS }` trong WHERE chặn race: nhiều request đoán sai gần như đồng thời không thể cùng đọc `attempts` dưới ngưỡng rồi cùng tăng, khiến giá trị vượt quá giới hạn. |
